...
Example: read a file, modify text, and preserve encoding/newlines.
...
Example: replace text while preserving the original file style.
...
Useful commands:
    ...
    file data/system/YesNoDialog.tjs

    xxd -g 1 data/startup.tjs | head
    xxd -g 1 data/system/YesNoDialog.tjs | head
...
Build the log helper manually:

    cd path/to/log-helper
    GOOS=windows GOARCH=amd64 go build -trimpath -ldflags="-H=windowsgui -s -w" -o log.exe .
...
| Kirikiri 2  | Best supported              |
| RPG Maker   | Detection / partial support |
